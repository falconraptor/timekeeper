...
public class calender extends JFrame {
	...
	private JPanel setgui () {
		...
		l.add(new JLabel(c.getDisplayName(Calendar.MONTH, Calendar.LONG, Locale.getDefault()), 0));
		p.get(0).add(left);
		...
		l.get(c.get(7) + ((c.get(Calendar.WEEK_OF_MONTH) - 1) * 7)).setText(c.get(5) + "");
		int min = 0, max = 0, m = c.get(Calendar.MONTH);
		if (m == 1 || m == 3 || m == 5 || m == 7 || m == 8 || m == 10 || m == 12) max = 31;
		else if (m == 2) {
			if (c.getActualMaximum(Calendar.DAY_OF_YEAR) > 365) max = 29;
			else max = 28;
		} else max = 30;
		for (int i = c.get(5) - 1; i > 0; i--) {
			l.get(c.get(7) + ((c.get(Calendar.WEEK_OF_MONTH) - 1) * 7) - (c.get(5) - i)).setText(i + "");
			if (i == 1) {
				min = c.get(7) + ((c.get(Calendar.WEEK_OF_MONTH) - 1) * 7) - (c.get(5) - i) - 1;
				int a = 0;
				while (min - a > 0) {
					try {
		...
		for (int i = c.get(5) + 1; i < l.size(); i++) {
			if (i <= max)
				l.get(c.get(7) + ((c.get(Calendar.WEEK_OF_MONTH) - 1) * 7) + (i - c.get(5))).setText(i + "");
			else try {
				l.get(c.get(7) + ((c.get(Calendar.WEEK_OF_MONTH) - 1) * 7) + (i - c.get(5))).setText(i - max + "");
				nm[i + min] = true;
			} catch (Exception e) {
				System.err.println(e);
			}
		...
		try {
			return new ActionListener() {
				public void actionPerformed (ActionEvent e) {
					...
					calfold = new File(calfold, c.get(Calendar.YEAR) + "");
					if (!calfold.exists()) calfold.mkdirs();
					calfold = new File(calfold, c.get(Calendar.MONTH) + 1 + "");
					if (!calfold.exists()) calfold.mkdirs();
					ed.place = n;
					ed.cal = calfold;
					ed.appear();
