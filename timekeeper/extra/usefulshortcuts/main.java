						...
						if (item.getLabel().equals(f.getcommand())) {
							if (item.getLabel().equals("Timekeeper")) {
								...
								if (!new File("timekeeper" + read + ".jar").exists())
									util.downloadfile(f.geturl(), "timekeeper" + read + ".jar");
								try {
							...
							if (!new File(f.getpath()).exists() && !f.geturl().equals(""))
								util.downloadfile(f.geturl(), f.getpath());
							try {
								if (Desktop.isDesktopSupported())
				...
				if (button.equals("Add")) {
					...
					if (stuff.equals("Chrome") || stuff.equals("Internet Explorer")) {
						...
						if (returnVal == JFileChooser.APPROVE_OPTION) {
							...
							try {
								file.add(new files(util.getrelativepath(chooser.getSelectedFile().getAbsolutePath()), name, ""));
							} catch (Exception e) {
								System.err.println(e);
							}
							lists.clear();
