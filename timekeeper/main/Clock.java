...
public class Clock extends JFrame {
	...
	public void clockmain () {
		...
		feedback.addActionListener(new ActionListener() {
			public void actionPerformed (ActionEvent evt) {
				try {
					Desktop.getDesktop().browse(new URL("https://github.com/falconraptor/timekeeper/issues").toURI());
				} catch (Exception e) {
					System.err.println(e);
				}
			}
	...
	private <T extends JComponent> void setcolors (T o) {
	...
	private void weekends (int daynum, Calendar time) {
		...
		if (daynum == 7) {
			...
			labels.get(4).setText("Not A School Day");
			donechecking = 1;
			calctime(time.get(Calendar.HOUR_OF_DAY), time.get(Calendar.MINUTE), time.get(Calendar.DAY_OF_WEEK) + 1, true);
		} else
			calctime(time.get(Calendar.HOUR_OF_DAY), time.get(Calendar.MINUTE), time.get(Calendar.DAY_OF_WEEK), false);
	}
	...
	private String[] checkholidays () {
		...
		int month = time.get(Calendar.MONTH) + 1, daymonth = time.get(Calendar.DAY_OF_MONTH);
	...
	private class UpdateUITask extends TimerTask {
		...
		public void run () {
			...
			boolean am = checkam(Integer.parseInt(hour));
